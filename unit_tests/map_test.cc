...
// заменить на lace::map
using MapInts = lace::map<int, int>;
using MapOnCreationObj = lace::map<int, IncreaseOnCreate>;
using MapIntString = lace::map<int, std::string>;
using MapStringInt = lace::map<std::string, int>;

// не менять на lace::map
...
TEST(MapAccesOperators, AtOperator1) {
  MapInts map = {{6, 10}};
  ASSERT_EQ(map.at(6), 10);
}

TEST(MapAccesOperators, BracketsOperator1) {
  MapInts map = {{6, 10}};
  ASSERT_EQ(map[6], 10);
}

TEST(MapAccesOperators, BracketsOperatorOnEmptyIntsMap1) {
  MapInts map;
  int i{};
  ASSERT_EQ(i, map[1]);
}

TEST(MapAccesOperators, BracketsOperatorOnEmptyStringMap1) {
  MapIntString map;
  std::string s;
  ASSERT_EQ(s, map[1]);
}

TEST(MapAccesOperators, AtOperatorOnEmptyMap1) {
  MapInts map;
  ASSERT_ANY_THROW(map.at(1));
}

TEST(MapIteratorTest, EmptyMap1) {
  MapInts map;
  ASSERT_EQ(map.begin(), map.end());
}

TEST(MapIteratorTest, OneElement1) {
  MapInts map = {{1, 1}};
  ASSERT_EQ(map.begin(), --map.end());
}

TEST(MapIteratorTest, SwapTwoNonEmpty1) {
...
TEST(MapIteratorTest, OneEmptyMapCheckNonEmpty1) {
  MapInts first = {{1, 1}};
  MapInts second;
...
TEST(MapIteratorTest, OneEmptyMapCheckFirstAndLoops1) {
  MapInts first = {{1, 1}};
  MapInts second;
...
TEST(MapIteratorTest, ForwardIteration1) {
...
TEST(MapCapacityTest, OneEmptySize1) {
  MapInts first = {{1, 1}};
  MapInts second;
...
TEST(MapCapacityTest, OneEmptyIsEmpty1) {
  MapInts first = {{1, 1}};
  MapInts second;
...
TEST(MapGeneralTest, InsertEraseSwapIterate1) {
...
TEST(MapGeneralTest, ExistingInsertTest1) {
  MapInts map = {{1, 3}};
...
TEST(MapGeneralTest, ObjCreationTest1) {
  IncreaseOnCreate::count = 0;
...
TEST(MapConstructorsTest, CopyConstructor1) {
...
TEST(MapConstructorsTest, MoveConstructor1) {
  OriginalMapInts orig = {{8, 2}, {1, 4}, {9, 1}};
  MapInts map = {{8, 2}, {1, 4}, {9, 1}};
...
TEST(MapInsertTest, InsertOrAssignEmpty1) {
  MapInts map;
...
TEST(MapInsertTest, InsertOrAssignNonEmptyExisting1) {
  MapInts map = {{1, 3}};
  auto pair = map.insert_or_assign(1, 1);
  ASSERT_EQ(1, map.size());
  ASSERT_EQ(1, map.begin()->second);
...
TEST(MapInsertTest, InsertOrAssignNonEmptyNonExisting1) {
...
TEST(MapInsertTest, InsertOrAssignNonEmptyNonExisting21) {
...
TEST(MapInsertTest, InsertOrAssignNonEmptyExisting21) {
  OriginalMapInts orig = {{1, 3}, {2, 2}, {3, 3}, {4, 4}};
...
TEST(MapInsertTest, InsertPairNonEmptyExisting1) {
...
TEST(MapInsertTest, InsertPairEmpty1) {
  MapInts map;
...
/* РАСКОММЕНТИРОВАТЬ ПОСЛЕ ПОДСТАНОВКИ lace::map в MapInts */

TEST(MapInsertTest, InsertNonEmptyExisting1) {
  OriginalMapInts orig = {{1, 3}, {2, 2}, {3, 5}, {4, 4}};
  MapInts map = {{1, 3}, {2, 2}, {3, 5}, {4, 4}};
  auto pair = map.insert(3, 3);
  ASSERT_TRUE(checkMapsEqual(map, orig));
...
TEST(MapInsertTest, InsertEmpty1) {
  MapInts map;
  auto pair = map.insert(1, 1);
  ASSERT_EQ(1, map.size());
  ASSERT_EQ(1, map.begin()->second);
...
TEST(MapGeneralTest, ClearNonEmpty1) {
  MapInts map = {{1, 3}, {2, 2}, {3, 5}, {4, 4}};
...
TEST(MapGeneralTest, ClearEmpty1) {
  MapInts map;
  ASSERT_NO_THROW(map.clear());
  ASSERT_EQ(0, map.size());
  ASSERT_EQ(map.begin(), map.end());
...
TEST(MapMergeTest, Overlapping1) {
...
TEST(MapMergeTest, NotOverlapping1) {
  MapInts first = {{1, 1}, {8, 8}, {9, 9}, {11, 11}};
...
TEST(MapMergeTest, IteratorOnNotOverlapping1) {
  MapInts first = {{1, 1}, {8, 8}, {9, 9}, {11, 11}};
  MapInts second = {{2, 2}, {3, 3}, {12, 12}};
...
TEST(MapMergeTest, IteratorOnOverlapping1) {
  MapInts first = {{1, 1}, {8, 8}, {9, 9}, {11, 11}};
  MapInts second = {{2, 2}, {3, 3}, {8, 8}, {12, 12}};
...
TEST(MapGeneralTest, InsertManySimpleTest1) {
  MapInts map = {{1, 0}, {4, 0}};
  OriginalMapInts orig = {{1, 0}, {2, 0}, {3, 0}, {4, 0}};
  std::pair<int, int> p1 = {2, 0};
  std::pair<int, int> p2 = {3, 0};
  std::pair<int, int> p3 = {3, 0};
  std::pair<int, int> p4 = {2, 0};
  map.insert_many(p1, p2, p3, p4);

  ASSERT_TRUE(checkMapsEqual(map, orig));
}

TEST(MapGeneralTest, InsertManyIteratorCheck1) {
  MapInts map = {{1, 0}, {4, 0}};
  OriginalMapInts orig = {{1, 0}, {2, 0}, {3, 0}, {4, 0}};
  ...
  std::pair<int, int> p4 = {2, 0};
  map.insert_many(p1, p2, p3, p4);
  // auto arr = map.insert_many({{2, 0}, {3, 0}, {3, 0}, {2, 0}}); // для
  // инициализации списком, раскомментировать метод в map
  iterator++;
...
TEST(MapGeneralTest, InsertManyReturnCheck1) {
  MapInts map = {{1, 0}, {4, 0}};
  auto iterator = map.begin();

  // Явно создаем пары перед передачей
  std::pair<int, int> p1 = {2, 0};
  std::pair<int, int> p2 = {3, 0};
  std::pair<int, int> p3 = {3, 0};
  std::pair<int, int> p4 = {2, 0};

  auto arr = map.insert_many(p1, p2, p3, p4);

  ASSERT_EQ(++iterator, arr[0].first);
  ASSERT_EQ(true, arr[0].second);
  ASSERT_EQ(++iterator, arr[1].first);
  ASSERT_EQ(true, arr[1].second);
...
TEST(MapIteratorTest, ConstAndNormalIterator1) {
  MapInts map = {{1, 2}};
  lace::map<int, int>::iterator ci = map.begin();
  lace::map<int, int>::const_iterator i = map.begin();

  ASSERT_EQ(i, ci);
}

TEST(MapIteratorTest, ConstIterator1) {
...
TEST(MapEraseBalanceTest, EraseByIteratorRandom1) {
  MapInts set;
  const int N = 10000;
